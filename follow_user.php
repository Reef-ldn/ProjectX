<!--This script handles following user functionality-->
<?php
session_start();
...
$action = $_GET['action'] ?? '';    //add or remove follow
...
if ($action == 'follow') {
  ...
  $sqlCheck = "SELECT * FROM follows WHERE follower_id='$follower_id' AND followed_id='$followed_id' ";
...
//If the user wants to 'unfollow'
  ...
  //delete row
  $deleteSql = "DELETE FROM follows WHERE follower_id ='$follower_id' AND followed_id = '$followed_id' ";
