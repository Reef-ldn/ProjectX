...
if(isset($_POST['submit_post'])) {    //if the post button is pressed.
  ...
  $stmt = $conn->prepare("INSERT INTO posts (user_id, post_type, file_path, text_content, created_at)
                          VALUES (?, ?, ?, ?, now())");
  ...
  $stmt->bind_param("isss", $user_id, $post_type, $file_path, $text_content); //"isss" represents the above
